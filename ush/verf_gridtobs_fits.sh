...
then
   ...
   echo "usage: verf_gridtobs.sh $model $vday $vcyc"
   err_exit
fi
...
#  export XLFUNIT_50=bufrout_${domain}
   ...
   ln -sf $DATA/$vcyc/prepda.${vdate} fort.20
   ln -sf $DATA/$vcyc/bufrout_${domain} fort.50
...
#       export XLFUNIT_50=prepfits.${domain}.${vdate}
       ...
       ln -sf $DATA/$vcyc/gridtobs.levcat.${domain} fort.11
       ln -sf $DATA/$vcyc/bufrout_${domain} fort.20
       ln -sf $PARMverf_gridtobs/verf_gridtobs.prepfits.tab${XCT} fort.22
       ln -sf $DATA/$vcyc/prepfits.${domain}.${vdate} fort.50
...
#       fi
       ...
       do
        ...
        ## Skip the first V01 section in the parameter file for the off-time cycles
         if [[ `expr $vcyc % 12` -eq 0 ]] || [[ $jstat -gt 1 ]] || [[ $model = "aqm" ]] || [[ $model = "pm1" ]] || [[ $model = "rtma" ]] || [[ $model = "satimg" ]] || [[ $model = "satimgb" ]] || [[ $model = "satimgx" ]]|| [[ $model = "ngac" ]] || [[ $model = "pm" ]] || [[ $model = "hirtma" ]] || [[ $model = "prrtma" ]] || [[ $model = "akrtma" ]] || [[ $model = "aqm1" ]]
         then
...
#
#       if [ $model = "namtest" -o $model = "namak" -o $model = "rap" -o $model = "rapak" -o $model = "conusnest" -o $model = "aknest" -o $model = "hrrr" -o $model = "hiresw" -o $model = "nssl4arw" ]
        if [ $model = "oyvey" ]
         then
...
## End of Fire wx on NAM/RUC verification
  ...
  if [ $SENDCOM = YES ]
  then
    cat $DATA/$vcyc/${domain}_${vday}${vcyc}.vsdb >>$COMVSDB/${model}/${modnam}_${vday}.vsdb
  fi
