...
C-----------------------------------------------------------------------
      SUBROUTINE getfct(irep,ibak,fhour,onlysf,subset,src)
...
c         thunob = thuni(irep,jbak)
	  ...
	  END IF
          ...
          if(ceilob.lt.bmiss) then
             if(src(:3).eq.'NAM'.or.src(:9).eq.'CONUSNEST'.or.
     *           src(:6).eq.'AKNEST'.or.src(:9).eq.'PRICONEST'
     *          .or.src(:10).eq.'HAWAIINEST') then
                bak2(10,2) = bak2(10,2) + ceilob * timw
             else
                bak2(10,2) = bak2(10,2) + ceilob * timw  -hdr(5)
             endif
          else
                bak2(10,2) = bmiss
          endif
