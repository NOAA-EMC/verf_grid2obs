...
C-----------------------------------------------------------------------
      SUBROUTINE getprofupr(iearth,sub,pob,std)
...
C       --------------------------------
        ...
        allocate(hni(mxr,mxb))
        allocate(transi(mxr,mxb))
        allocate(wnd80i(mxr,mxb))
        allocate(ceili(mxr,mxb))
...
C       ------------------------------------
        ...
        DO 10 n = 1, nrep
      if(sub(n)(:6).eq.'ADPUPA'.or.sub(n)(:6).eq.'AIRSND') then
          do i=1,xlev(n)
           xobd(i)=xdft(1,i,n)
           yobd(i)=xdft(2,i,n)
           pobd(i)=pob(i,n)
...
c    *               thun(kxi(n)+1,kyj(n)+1)
     ...
     +                cdtz(kxi(n)+1,kyj(n)+1)
           IF ( chk ( mskhaines(i,j), mskhaines(i1,j), mskhaines(i,j1),
     +                mskhaines(i1,j1) ) ) hni(n,ibak) = wtsw(n) *
     +                haines(kxi(n),kyj(n)) + wtse(n) *
     +                haines(kxi(n)+1,kyj(n)) + wtnw(n) *
     +                haines(kxi(n),kyj(n)+1) + wtne(n) *
     +                haines(kxi(n)+1,kyj(n)+1)
           IF ( chk ( msktrans(i,j), msktrans(i1,j), msktrans(i,j1),
     +                msktrans(i1,j1) ) ) transi(n,ibak) = wtsw(n) *
     +                trans(kxi(n),kyj(n)) + wtse(n) *
     +                trans(kxi(n)+1,kyj(n)) + wtnw(n) *
     +                trans(kxi(n),kyj(n)+1) + wtne(n) *
     +                trans(kxi(n)+1,kyj(n)+1)
           IF ( chk ( mskwnd80(i,j), mskwnd80(i1,j), mskwnd80(i,j1),
     +                mskwnd80(i1,j1) ) ) wnd80i(n,ibak) = wtsw(n) *
     +                wnd80(kxi(n),kyj(n)) + wtse(n) *
     +                wnd80(kxi(n)+1,kyj(n)) + wtnw(n) *
     +                wnd80(kxi(n),kyj(n)+1) + wtne(n) *
     +                wnd80(kxi(n)+1,kyj(n)+1)
           IF ( chk ( mskceil(i,j), mskceil(i1,j), mskceil(i,j1),
     +                mskceil(i1,j1) ) ) ceili(n,ibak) = wtsw(n) *
     +                ceil(kxi(n),kyj(n)) + wtse(n) *
     +                ceil(kxi(n)+1,kyj(n)) + wtnw(n) *
     +                ceil(kxi(n),kyj(n)+1) + wtne(n) *
     +                ceil(kxi(n)+1,kyj(n)+1)
