...
C       ------------------------------------------
	  ...
	  lstop = 1
          ...
          qob = qsi(irep,jbak)
          oob1 = oz1i(irep,jbak)
          print*,'irep,jbak,oob1=',irep,jbak,oob1
          oob8 = oz8i(irep,jbak)

         if (subset(:6).ne.'AIRNOW') then
...
c==      elseif (subset(:6).eq.'AIRNOW') then
c==       IF (oob.lt.bmiss) THEN
c               bak(3,2) = bak(3,2) + oob * timw * 1.e-9
c               bak(3,2) = oob * 1.e-9
c==       ELSE
c==             bak(3,2) = bmiss
c==       END IF
...
c         IF (pli.lt.bmiss) bak(10,l)=bak(10,l) + pli * timw
         ...
         elseif (subset(:6).eq.'AIRNOW') then
          if(l.eq.1) then
            IF (oob1.lt.bmiss) THEN
                bak(3,l) = bak(3,l) + oob1 * timw * 1.e-9
c==             bak(3,l+1) = bak(3,l+1) + oob8 * timw * 1.e-9
c               bak(3,l) = oob * 1.e-9
            ELSE
                bak(3,l) = bmiss
            END IF
            IF (oob8.lt.bmiss) THEN
c==             bak(3,l) = bak(3,l) + oob1 * timw * 1.e-9
                bak(3,l+1) = bak(3,l+1) + oob8 * timw * 1.e-9
c               bak(3,l) = oob * 1.e-9
            ELSE
                bak(3,l+1) = bmiss
            END IF
