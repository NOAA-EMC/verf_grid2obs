...
C     CHECK FOR BEST CAPE
     ...
     *      and.nlv.le.2) then
     ...
     +              THEN
           ...
           RETURN
         END IF
C     CHECK FOR 0-6 KM WIND SHEAR
      ELSE IF (nchrvarbl(ivr).eq.5.and.namvarbl(ivr).eq.'SHR06'.
     *      and.nlv.le.2) then
        IF (obs4(7,1,nprp).lt.bmiss.and.obs4(7,1,nfcs).lt.bmiss)
     +              THEN
           obsval = obs4(7,1,nprp)
           forcst = obs4(7,1,nfcs)
           igotdata = 0
           RETURN
         END IF
C     CHECK FOR BROOKS CRAVEN SEVERE PARAMETER
      ELSE IF (nchrvarbl(ivr).eq.5.and.namvarbl(ivr).eq.'BCSVR'.
     *      and.nlv.le.2) then
        IF (obs4(8,1,nprp).lt.bmiss.and.obs4(8,1,nfcs).lt.bmiss)
     +              THEN
           obsval = obs4(8,1,nprp)
           forcst = obs4(8,1,nfcs)
           igotdata = 0
           RETURN
         END IF
C     CHECK FOR SCREENED CAPE
      ELSE IF (nchrvarbl(ivr).eq.5.and.namvarbl(ivr).eq.'SCAPE'.
     *      and.nlv.le.2) then
        IF (obs4(9,1,nprp).lt.bmiss.and.obs4(9,1,nfcs).lt.bmiss)
     +              THEN
           obsval = obs4(9,1,nprp)
           forcst = obs4(9,1,nfcs)
           igotdata = 0
           RETURN
         END IF
C     CHECK FOR 2nd SCREENED CAPE
      ELSE IF (nchrvarbl(ivr).eq.6.and.namvarbl(ivr).eq.'SCAPE2'.
     *      and.nlv.le.2) then
        IF (obs4(11,1,nprp).lt.bmiss.and.obs4(11,1,nfcs).lt.bmiss)
     +              THEN
           obsval = obs4(11,1,nprp)
           forcst = obs4(11,1,nfcs)
c          if (fhour(ifh).eq.0) 
c    *       print*,'stnid,obsval,forcst=',stnid,obsval,forcst
           igotdata = 0
           RETURN
         END IF
C     CHECK FOR 2nd BCSVR
      ELSE IF (nchrvarbl(ivr).eq.6.and.namvarbl(ivr).eq.'BCSVR2'.
     *      and.nlv.le.2) then
        IF (obs4(12,1,nprp).lt.bmiss.and.obs4(12,1,nfcs).lt.bmiss)
     +              THEN
           obsval = obs4(12,1,nprp)
           forcst = obs4(12,1,nfcs)
           igotdata = 0
           RETURN
         END IF
C     CHECK FOR HAINES INDEX
      ELSE IF (nchrvarbl(ivr).eq.6.and.namvarbl(ivr).eq.'HAINES'.
     *      and.nlv.le.2) then
        IF (obs4(10,1,nprp).lt.bmiss.and.obs4(10,1,nfcs).lt.bmiss)
     +              THEN
           obsval = obs4(10,1,nprp)
           forcst = obs4(10,1,nfcs)
           igotdata = 0
           RETURN
         END IF
C     CHECK FOR CINH
...
C      CHECK FOR MIXHT
     ...
     *    and.nlv.le.2 ) THEN
     ...
     +               THEN
            ...
            RETURN
          ENDIF
C     CHECK FOR TRANSPORT WIND
      ELSE IF (nchrvarbl(ivr).eq.5.and.namvarbl(ivr).eq.'TRANS'.
     *    and.nlv.le.2 ) THEN
         IF (obs5(3,1,nprp).lt.bmiss.and.obs5(3,1,nfcs).lt.bmiss)
     +               THEN
            obsval = obs5(3,1,nprp)
            forcst = obs5(3,1,nfcs)
c           print*,'transport wind obsval,forcst=',obsval,forcst
            igotdata = 0
            RETURN
          ENDIF
C     CHECK FOR VENTILATION RATE
      ELSE IF (nchrvarbl(ivr).eq.4.and.namvarbl(ivr).eq.'VENT'.
     *    and.nlv.le.2 ) THEN
         IF (obs5(4,1,nprp).lt.bmiss.and.obs5(4,1,nfcs).lt.bmiss)
     +               THEN
            obsval = obs5(4,1,nprp)
            forcst = obs5(4,1,nfcs)
            igotdata = 0
            RETURN
          ENDIF
C     CHECK FOR 80-M WIND
      ELSE IF (nchrvarbl(ivr).eq.6.and.namvarbl(ivr).eq.'WIND80'.
     *    and.nlv.le.2 ) THEN
         IF (obs5(5,1,nprp).lt.bmiss.and.obs5(5,1,nfcs).lt.bmiss)
     +               THEN
            obsval = obs5(5,1,nprp)
            forcst = obs5(5,1,nfcs)
            igotdata = 0
            RETURN
          ENDIF
C     CHECK FOR TMAX
...
c     CHECK FOR CEILING
      else if (nchrvarbl(ivr).eq.7.and.namvarbl(ivr).eq.'CEILING') then
         if (obs2(10,nlv,nprp).lt.bmiss.and.obs2(10,nlv,nfcs).lt.bmiss)
     *        THEN
          obsval = obs2(10,nlv,nprp)
          forcst = obs2(10,nlv,nfcs)
          igotdata = 0
          return
        endif
...
C     CHECK FOR NON-MISSING U-&V-COMP WITH PROPER QUALITY MARK
     ...
     +                bmiss) THEN
	    ...
	    END IF
          ...
          END IF
        END IF
      ELSE IF (nchrvarbl(ivr).eq.4.and.namvarbl(ivr).eq.'WDIR') THEN
        IF ((iqmod(iob).eq.1.and.qms(7,nlv).lt.3.).or.(iqmod(iob).eq.2
     +              .and.qms(7,nlv).ge.3.)) THEN
          IF (obs(7,nlv,nprp).lt.bmiss.and.obs(7,nlv,nfcs).lt.bmiss.and.
     +                obs(8,nlv,nprp).lt.bmiss.and.obs(8,nlv,nfcs).lt.
     +                bmiss) THEN
            ...
            ELSE
                ...
                vforcst = vf
            END IF
            call w3fc05(uobsval,vobsval,dir1,spd1)
            call w3fc05(uforcst,vforcst,dir2,spd2)
            obsval=dir1
            forcst=dir2
            igotdata = 0
            RETURN
