...
C
      ...
      REAL*8 hdr(10), obs(10,255,mxb), obs2(10,255,mxb),
...
c     data obstr /'SRC FHR POB QOB TOB ZOB UOB VOB PMO PBL CAPE CINH LI'/
      data obstr2 /'MXTM MITM TDO HOVI TOCC MXGS THI TCH CDBZ CEIL'/
c     data obstr3 /'CAPE CINH LI TROP PWO BCAPE HAINES'/
      data obstr4 /'TKEPBL RIPBL TRANS VENT WIND80'/
C
...
c     obstr='SRC FHR POB QOB TOB ZOB UOB VOB PMO PBL CAPE CINH LI'
      ...
      obstr3='CAPE CINH LI TROP PWO BCAPE SHR06 BCSVR SCAPE HAINES
     * SCAPE2 BCSVR2'
      print*,'obstr=',obstr
   10 CONTINUE
      call datelen(10)
...
c               IF (subset(:6).eq.'ADPSFC'.or.subset(:6).eq.'SFCSHP')
     ...
     *              or.subset(:6).eq.'MSONET'.or.subset(:6).eq.'SATWND')
     +		THEN
	            ichk = 1
		    lstart (iob) = 2
		END IF
              ELSE IF (subset(:6).eq.namvfyobs(iob)(:6)) THEN
                ichk = 1
                IF (subset(:6).eq.'ADPSFC'.or.subset(:6).eq.'SATWND'.or.
...
c               print*,'after trop pwo'
     ...
     *             'GPSIPW'.
     *                      or.subset(:6).eq.'AIRSND') then
c                     print*,'obs4 read'
                      obs4=bmiss
                      nlev2=nlev
                      nlev=1
c                     print*,'before ufbin3'
                      call ufbin3(lunin,obs4,12,1,mxb,nlev,nevn,
     *                  obstr3)
c                     print*,'nevn=',nevn
c                     print*,'subset,stnid=',subset,stnid
c                     do n=1,nevn
c                      print*,'obs4(7,1,n)=',obs4(7,1,n)
c                     enddo
c                      do j=1,2
c                      do i=1,6
...
c                     print*,'after ufbin3 of obs4'
              ...
              if(subset(:6).eq.'ADPUPA'.
     *                      or.subset(:6).eq.'AIRSND') then
c                     print*,'obs5 read'
                      obs5=bmiss
                      nlev2=nlev
                      nlev=1
                      call ufbin3(lunin,obs5,5,1,mxb,nlev,nevn,
     *                  obstr4)
                      nlev=nlev2
              endif
...
c------- for profile integrated variables assigned to first obs level ------
                           if (subset(:6).eq.'AIRSND') then
                                  if(probs.ne.bmiss) probs=1000
                            endif
                        kat = nint(qms(1,nlv))
...
c    *   and.namfcst(ifh).eq.'00'.and.namvarbl(ivr).eq.'V'.
c    *   and.namlevel(ilv).eq.'SFC') then
c        print*,'count=',count(ifh,ivr,ilv,iar,iob,ist)
c        print*,'stnid,subset=',stnid,subset,namfcst(ifh)
