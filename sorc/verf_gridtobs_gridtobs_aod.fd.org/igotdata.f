      FUNCTION igotdata(obsval,forcst,obs,obs2,obs4,obs5,obst,qms,nevn,
     *            nlv,imodel,ifh,
...
C       NEXT CHECK FOR  B O T H  FMODEL  A N D  FHOUR
     ...
     +      nint(obs(2,nlv,n)).eq.nint(fhour(ifh))) THEN
          nfcs = n
        END IF
        if(subset(:8).eq.'NC008041'.and.nint(obst(2,1,n)).
     *      eq.nint(fhour(ifh))) then
          nfcs = n
        endif
      END DO
      igotdata = -1
...
c     print*,'namvarbl(ivr)=',namvarbl(ivr),subset
     ...
     *        THEN
         ...
         if (obs4(5,2,nprp).lt.bmiss.and.obs4(5,2,nfcs).lt.bmiss.
