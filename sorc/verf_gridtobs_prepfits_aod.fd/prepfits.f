...
C     8 & 9 - reserved
      ...
      if(fit(7)) then
        isat=1
        print*,'Using bufr tanks for AOD'
      endif
...
C     ------------------------------------------------------------
      ...
      print*,'idate after 1st datebf=',idate
...
C     
      IF (idate.ge.0) THEN
        if(isat.eq.1) then
        CALL ufbxy3_goes(lubfi,mate,2,mxr)
        else
...
c     print*,'before the read 5'
      DO n = 1, mxb+1
        READ (5,*,END=10) fhours(n), src(n), file(n)
        print*,'file,fhour=',file(n),fhours(n)
        READ (5,*,END=10) fhours(n), src(n), fndx(n)
      END DO

c     print*,'after the read 5'
...
c       call baopen(21,file(ibak),ishl)
c       call baopen(23,fndx(ibak),ishi)
        call baopenr(luges,file(ibak),ishl)
        call baopenr(lugbi,fndx(ibak),ishi)
        print*,'luges,file(ibak)=',luges,file(ibak)
        print*,'file(ibak),ishl,ishi=',file(ibak),ishl,ishi
c       ishl=0
c       ishi=0
...
C       
        nofo(ibak) = 1
c       call baclose(21,iret)
c       call baclose(23,iret)
      END DO  
...
C           -----------------------------------------------
            ...
            if(subset(:8).eq.'NC008041') then
            headrt='YEAR MNTH DAYS HOUR MINU SECO CLATH CLONH'
            call ufbint(lubfo,hdrt,10,1,iret,headrt)
            else
...
c 
            ...
            if(subset(:8).eq.'NC008041') then
             call ufbint(lubfo,obst,1,1,iret,'OPTD')
            endif
...
c            
c           if(subset(:8).eq.'NC008041') then
c           do i=1,6
c             print*,'irep,obst(1,1),bakt(1,1)=',irep,
c    *           obst(1,1),bakt(1,1),fhour,hdrt(7),hdrt(8)
c           enddo
c           endif
            if(subset(:8).eq.'NC008041') then
             call ufbint(lubfo,bakt,1,1,iret,'OPTD')
            endif
