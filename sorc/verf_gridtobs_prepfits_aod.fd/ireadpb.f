...
C  ------------------------------------------
      ...
      if(subset(:8).eq.'NC008041') then
       headrt='YEAR MNTH DAYS HOUR MINU SECO CLATH CLONH SAID'
       call ufbint(lunit,hdrt,9,1,iret,headrt)
       call ufbint(lunit,obst,1,1,nlev,'OPTD')
c      print*,'hdrt(7),hdrt(8)=',hdrt(7),hdrt(8)
c      do i=1,9
c        print*,'i,hdrt(i)=',i,hdrt(i)
c      enddo
c      do i=1,2
c        print*,'obst(1,i)=',obst(1,i)
c      enddo
       return
      endif
