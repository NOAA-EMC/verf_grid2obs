...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'parm.inc'
      character*8 sub(mxr)
      real*8 xobd(mxr), yobd(mxr), pobd(mxr)
      real*8 xid(mxr), yjd(mxr)
      real*8 pob(255,mxr)
      real*8 xi,yj
      real*8 dx,dy,dxm
      real*8 D001,D1000
...
c           dxm = dxx * 1000.
      ...
      if(sub(n)(:6).eq.'ADPUPA') then
            do i=1,xlev(n)
            CALL w3fb11a(yobd(i),xobd(i),alat1,elon1,dxm,elonv,alatan1,
     *        xid(i),yjd(i))
	    ...
	    rmx2 = SIN ( theta )
            enddo
             ...
             CALL w3fb11a(yob,xob,alat1,elon1,dxm,elonv,alatan1,
     *        xi,yj)
...
c    *               thun(kxi(n)+1,kyj(n)+1)
     ...
     +                cdtz(kxi(n)+1,kyj(n)+1)
       if(cdti(n,ibak).lt.0.0) cdti(n,ibak)=bmiss
           IF ( chk ( mskaod(i,j), mskaod(i1,j), mskaod(i,j1),
     +                mskaod(i1,j1) ) ) aodi(n,ibak) = wtsw(n) *
     +                aod(kxi(n),kyj(n)) + wtse(n) *
     +                aod(kxi(n)+1,kyj(n)) + wtnw(n) *
     +                aod(kxi(n),kyj(n)+1) + wtne(n) *
     +                aod(kxi(n)+1,kyj(n)+1)
            print*,'aod(kxi(n),kyj(n))=',aod(kxi(n),kyj(n))
            print*,'ibak,aodi(n,ibak)=',ibak,aodi(n,ibak)
            print*,'kxi(n),kyj(n)=',kxi(n),kyj(n)
       ENDIF  !  kxi.ne.0
