      FUNCTION inarea(imod, sid, xobi, yobi, iar, rm1, rm2, namarea ,
     *   namfcst,namvarbl)
C**********************************************************************
...
C*
     ...
     +            ig104(147,110), regions
      ...
      CHARACTER*8 stnlist
      character*24 namarea,namfcst,namvarbl
      real*8 xobi,yobi
      real xob,yob
...
C
C  FIRE WEATHER GRID INFORMATION
C
      if(namarea(1:4).eq.'G255') then
       read(namfcst(1:2),'(i2)') inamfcst
       if(mod((inamfcst-ifirst),6).ne.0.or.inamfcst.gt.36.
     *     or.inamfcst.lt.ifirst) return
       do i=1,icount
         if(nint(fcsthrf(i)).eq.inamfcst) then
          imax(iar)=imaxf(i)
          imin(iar)=1
          jmax(iar)=jmaxf(i)
          jmin(iar)=1
          ...
          dyy(iar)=dyyf(i)
          elonv(iar)=elonvf(i)
          alatan(iar)=alatanf(i)
          latlong(iar)=latlongf(i)
          lambert(iar)=lambertf(i)
          polarstereo(iar)=polarstereof(i) 
          isave=i
         goto 555
         endif
       enddo
...
C
          if(namarea(1:4).eq.'G255') then
            ...
            kyj1=kyj-1
            kyj2=kyj+1
            if(namvarbl(1:4).ne.'VWND'.and.(imask(kxi,kyj,isave).eq.0
     *          .or.imask(kxi2,kyj,isave).eq.0 
     *          .or.imask(kxi,kyj2,isave).eq.0
     *          .or.imask(kxi2,kyj2,isave).eq.0)) then
              inarea = -1
              return
            endif
            if(namvarbl(1:4).eq.'VWND'.and.(imasku(kxi,kyj,isave).eq.0
     *          .or.imasku(kxi2,kyj,isave).eq.0
     *          .or.imasku(kxi,kyj2,isave).eq.0
     *          .or.imasku(kxi2,kyj2,isave).eq.0)) then
              inarea = -1
              return
            endif
           endif
C
C END OF FIRE WX MASK CHECK
C
...
C
	rm1 = 1.0
	rm2 = 0.0
	IF ( nmbgrd (imod) .gt. 0 .and. inarea .eq. 0 ) THEN
	  IF ( cenlon (imod) .gt. 180.0 ) THEN
		rlamc = cenlon (imod) - 360.0
	  ELSE
		rlamc = cenlon (imod)
	  END IF
	  IF ( xob .gt. 180. ) THEN
	  ...
	  ELSE
		rlam = xob
	  END IF
	  theta = ( rlam - rlamc ) * factor * concon (imod)
	  rm1 = COS ( theta )
	  rm2 = SIN ( theta )
	END IF
C*
