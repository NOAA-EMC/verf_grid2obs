...
C
      ...
      REAL*8 hdr(10), obs(9,255,mxb), obs2(10,255,mxb),
...
c     data obstr /'SRC FHR POB QOB TOB ZOB UOB VOB PMO PBL CAPE CINH LI'/
      ...
      data obstr3 /'PBL CAPE CINH LI TROP PWO'/
...
c
      ...
      read(5,*) icyc
      print*,'cycle=',icyc
       call baopen(31,"firemask1.grb",iret)
       call baopen(32,"firemask1i.grb",iret)
       call baopen(33,"firemask2.grb",iret)
       call baopen(34,"firemask2i.grb",iret)
       call baopen(35,"firemask3.grb",iret)
       call baopen(36,"firemask3i.grb",iret)
       call baopen(37,"firemask4.grb",iret)
       call baopen(38,"firemask4i.grb",iret)
       call baopen(39,"firemask5.grb",iret)
       call baopen(40,"firemask5i.grb",iret)
       call baopen(41,"firemask6.grb",iret)
       call baopen(42,"firemask6i.grb",iret)
       call baopen(43,"firemask7.grb",iret)
       call baopen(44,"firemask7i.grb",iret)
...
C     
     ...
     +            numvarbl,numlevel,numvector)
      ...
      END IF
c
c READ IN THE FIRE WX MASK DATA ONCE SO IT IS DONE.  SAVE ALL MASK DATA IN
c SEPARATE ARRAYS SO THAT IN INAREA THE OB CAN DETERMINE WHETHER IT IS IN
c THE FIRE WX DOMAIN THAT IS USED AT THAT PARTICULAR HOUR.

      print*,'numvfyobs=',numvfyobs
      print*,'namfcst=',namfcst
ccc     call maskdef(namfcst,numfcst)
C     
...
C                 
                  DO 70 iar = 1, numarea
c                   IF (inarea(imodel,stnid,hdr(2),hdr(3),iar,rm1,rm2,
c    *                  namarea)
c    +			.eq.0) THEN
c                   ifh=1
c                   namfcst(ifh)="12"
c                   IF (inarea(imodel,stnid,hdr(2),hdr(3),iar,rm1,rm2,
c    *                  namarea,namfcst(ifh))
c    +                  .eq.0) THEN
...
c          if(subset(:6).eq.'MSONET') then
c          print*,'subset,stnid=',subset,stnid
c          do i=1,nevn
c           print*,'obs(9,1,i)=',i,obs(9,1,i)
c          enddo
c          endif
...
c      icnt=0
     ...
     *                  namarea,namfcst(ifh),namvarbl(ivr))
     +                  .eq.0) THEN
c                   IF (inarea(imodel,stnid,hdr(2),hdr(3),iar,rm1,rm2,
c    *                  namarea)
c    +                  .eq.0) THEN
...
c     if(namvfyobs(iob).eq.'MSONET'.and.namarea(iar).eq.'G255'.
c    *   and.namfcst(ifh).eq.'12'.and.namvarbl(ivr).eq.'VWND') then
