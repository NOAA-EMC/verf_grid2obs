...
C     --------------------------------------------------------
      ...
      DO n = 1, mxb
        READ (5,'(A8,1X,A20,1X,A20)',END=10) src(n),file1(n),file8(n)
        READ (5,'(A8,1X,A20,1X,A20)',END=10) src(n),fndx1(n),fndx8(n)
      END DO
...
C     ------------------
   ...
   20 CONTINUE
      STOP
   30 CALL errmsg("PREPFITS - BAD OR MISSING/INCOMPLETE PREPBUFR DATA")
      CALL ERREXIT(1)
      END
