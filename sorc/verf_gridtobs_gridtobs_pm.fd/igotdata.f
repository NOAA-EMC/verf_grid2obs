...
C     CHECK FOR CAPE
     ...
     +              THEN
         ...
         END IF
C     CHECK FOR COPO
       print*,ivr,nchrvarbl(ivr),namvarbl(ivr)
     ...
     +               THEN
           print*,'---1',nlv,nprp,(obs(ii,nlv,nprp),ii=1,5)
           print*,'---2',nlv,nfcs,(obs(ii,nlv,nfcs),ii=1,5)
            obsval = obs(5,nlv,nprp)
            forcst = obs(5,nlv,nfcs)
c           ozaverage = obs(3,nlv,nprp)
            igotdata = 0
            if(ozaverage.ne.obs(3,nlv,nprp)) igotdata = -1
            RETURN
          ENDIF
C     CHECK FOR COPOPM
       print*,ivr,nchrvarbl(ivr),namvarbl(ivr)
      ELSE IF (nchrvarbl(ivr).eq.6.and.namvarbl(ivr).eq.'COPOPM' )
     +   THEN
           print*,'---obsval=',(obs(ii,nlv,nprp),ii=3,5)
           print*,'---forcst=',(obs(ii,nlv,nfcs),ii=3,5)
         IF (obs(5,nlv,nprp).lt.bmiss.and.obs(5,nlv,nfcs).lt.bmiss
