     ...
     *            nlv,imodel,ifh,
     +            ivr,ilv,iob,rm1,rm2,subset,stnid)
C
      INCLUDE 'parm.inc'
C
...
C
      real*8 obs(9,255,mxb), obs2(10,255,mxb), 
...
C      CHECK FOR LI
     ...
     *    and.nlv.le.2) THEN
         IF (obs4(4,1,nprp).lt.bmiss.and.obs4(4,1,nfcs).lt.bmiss)
     +               THEN
            obsval = obs4(4,1,nprp)
            forcst = obs4(4,1,nfcs)
            igotdata = 0
            RETURN
          ENDIF
C      CHECK FOR PBL
       ELSE IF (nchrvarbl(ivr).eq.3.and.namvarbl(ivr).eq.'PBL'.
     *    and.nlv.le.2) THEN
...
c    CHECK FOR PRECIPITABLE WATER
     ...
     *        THEN
         ...
         if (obs4(6,2,nprp).lt.bmiss.and.obs4(6,2,nfcs).lt.bmiss.
     *        and.subset(:6).eq.'GPSIPW'.and.nlv.eq.2)
     *        THEN
          obsval = obs4(6,2,nprp)
          forcst = obs4(6,2,nfcs)
          igotdata = 0
          return
        endif
...
C     CHECK FOR NON-MISSING TEMPERATURE WITH PROPER QUALITY MARK
     ...
     +		    (iqmod(iob).eq.2.and.qms(4,nlv).ge.3.)) THEN
                    print*,'obs(4,nlv,nprp)=',obs(4,nlv,nprp)
