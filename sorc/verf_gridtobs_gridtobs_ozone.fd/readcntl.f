...
c           print*,'length of ob typ=',nchrvfyobs(i)
            print*,'i,namvfyobs(i)=',i,namvfyobs(i)
         ENDDO
      ENDIF
C     
...
C     
      READ(5,*) numarea,namarea(1)
c     read(5,'(i5,2x,a24)') numarea,namarea(1)
...
C     
      READ(5,'(i5,2x,a24)') numstat,namstat(1)
...
C     
      READ(5,'(i5,2x,a24)') numvarbl,namvarbl(1)
      nchrvarbl(1)=len(trim(namvarbl(1)))
      print*,'namvarbl=',namvarbl(1)
...
c     READ THE REST OF THE VARIABLES IF NECESSARY

      print*,'numvarbl=',numvarbl
      IF ( numvarbl.gt.1) THEN
         DO i = 2,numvarbl
            READ(5,'(2x,a24)') namvarbl(i)
c           read(5,*) namvarbl(i)
            print*,'namvarbl(i)=',i,namvarbl(i)
...
C     
        READ(5,'(i5,2x,a24)') numlevel,namlevel(1)
        nchrlevel(1) = LEN(TRIM(namlevel(1)))
        print*,'namlevel=',namlevel(1)
C
