...
c     DATA obstr /'SRC FHR POB QOB TOB ZOB UOB VOB PMO CAPE CINH LI'/
      DATA obstr /'SRC FHR TPHR QCIND COPO                         '/
C
...
c==   REAL*8  FOT(NUMTHR,IHR),HOT(NUMTHR,IHR),OOT(NUMTHR,IHR)
     ...
     +          tg(mxfcst,mxvrbl,maxlvl,mxarea,maxobs)
      DATA THRESH /50., 60., 65., 70., 75., 85., 105., 125., 150./
C--
      call datelen(10)
   10 CONTINUE
...
C         
     ...
     +                      'PROFLR'.or.subset(:6).eq.'AIRNOW') THEN
			    ichk = 1
			    IF (subset(:6).eq.'ADPSFC'.or.
     +                          subset(:6).eq.'AIRNOW'.or.
     ...
     +		THEN
		...
		END IF
              ELSE IF (subset(:6).eq.namvfyobs(iob)(:6)) THEN
                ichk = 1
		IF (subset(:6).eq.'ADPSFC'.or.
     +		    subset(:6).eq.'SFCSHP'.or.subset(:6).eq.'AIRNOW') 
     +              lstop (iob) = 1
              END IF
   20       CONTINUE
...
c              do kk=1,2
c              print*,'===',kk,stnid,(obs(ii,1,kk),ii=2,5)
c              enddo
...
C                           
     ...
     +                         namvarbl(ivr).eq.'OZON/8'.or.
     +                         namvarbl(ivr).eq.'OZMX/1'.or.
     +                         namvarbl(ivr).eq.'OZMX/8') then
                                if(namvarbl(ivr)(5:6).eq.'/1')
     +                             ozaverage=-1
                                if(namvarbl(ivr)(5:6).eq.'/8')
     +                             ozaverage=-8
                                namozone=namvarbl(ivr)
                                namvarbl(ivr)='COPO'
                                nchrvarbl(ivr)=4
                            endif
...
c                           print*,'---',ozaverage,namvarbl(ivr)
...
c      icnt=icnt+1
c      print*,'icnt=',icnt
       print*,'obsval=',obsval,'forcst=',forcst,stnid,ozaverage,iar
...
c                           if(namvarbl(ivr).eq.'COPO'.and.
c    +                                     ozaverage.eq.-1)then
c                                          namvarbl(ivr)='OZON/1'
c                                          nchrvarbl(ivr)=6
c                           endif
c                           if(namvarbl(ivr).eq.'COPO'.and.
c    +                                     ozaverage.eq.-8)then
c                                          namvarbl(ivr)='OZON/8'
c                                          nchrvarbl(ivr)=6
c                           endif

                            if(namvarbl(ivr).eq.'COPO') then
                                           namvarbl(ivr)=namozone
                                           nchrvarbl(ivr)=6
                            endif
...
c1201        FORMAT (A,1x,'AIRNOW',1x,A,1x,' FHO>',f4.0,1x,
