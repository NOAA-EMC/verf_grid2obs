...
c     obstr='TPHR QCIND COPO                           '
c--   obstr='TPHRPM QCINPM COPOPM                      '
...
c         PRINT*,'NLEV,CAT: ',NLEV,(CAT(N),N=1,NLEV)
          PRINT*,'NLEV=1,OBS: ',(OBS(I,1),I=1,10)
...
c    +                    'SFCSHP'.or.subset(:6).eq.'ANOWPM')) 
     +                    'SFCSHP')) 
     +                     cat(mlev) = 0
              DO i = 1, 10
                obs(i,mlev) = obs(i,l)
...
c    +   subset (:6) .eq. 'SFCSHP'.or.subset(:6).eq.'ANOWPM' ) THEN
...
C           ----------------------------------------------
              ...
              if(obs(3,1).ne.bmiss)
     +          write(102,1556) hdr(3), hdr(2), obs(3,1)*1e9, staid
...
c==         if(subset(:6).eq.'ANOWPM'.and.ozone_ave.eq.'8HR_AV') then
c==            obs(:,1)=obs(:,2)
c==         endif
...
C           ----------------------------------------------------------
     ...
     +                      valix
              ...
              IF (valid) THEN
                irepv(n) = irepv(n) + 1
	      if (subset(:6).eq.'ANOWPM') onlysf = .true.
                print*,'before getfct, fhour=',fhour
                CALL getfct(irep,n,fhour,onlysf,subset)
                print*,'after getfct, fhour=',fhour
                if (subset(:6).ne.'ADPUPA') then
                  do iii=8,10
                    bak(iii,1)=bmiss
                  enddo
                ...
                DO l = 1, nlev
                  cnf(1,l) = src(n)
                  print*,'snf(1,l)=',snf(1,l)
                  snf(2,l) = fhours(n)
                  print*,'snf(2,l)=',snf(2,l)
                END DO
                CALL ufbint(lubfo,snf,2,nlev,iret,'SRC FHR')
c               do j=1,50
...
c               enddo
            if(subset(:6).eq.'ANOWPM')then
              ...
              print*,'ANOWPM==',obs(3,1),bak(3,1)

              if(obs(3,1).ne.bmiss)
     +          write(101,1556) hdr(3), hdr(2), obs(3,1)*1e9, staid
c             if(obs(1,1).eq.-8.and.obs(3,1).ne.bmiss)
c    +          write(108,1556) hdr(3), hdr(2), obs(3,1)*1e9, staid
